...
import {useAddOntologicalDiseaseDescriptionMutation} from "../../../generated/graphql";
...
const DescriptionEditor : React.FC<Props> = ({description,set_editing,es_ID, es_field,ontodisease_ID,refetch}) => {
    ...
    const [addOntologicalDiseaseDescriptionMutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useAddOntologicalDiseaseDescriptionMutation({variables:{id:'', old_es_id:'',
    ...
    async function extracted(pmids: Array<string>) {
        const mutation_object = get_description_mutation_object(ontodisease_ID, es_ID, es_field, description_string.current, user_ID, pmids)
        await addOntologicalDiseaseDescriptionMutation({variables: mutation_object})
    }
