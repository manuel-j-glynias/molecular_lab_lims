...
import {EditableBoolean,ExtendedBoolean,useVariantRegionAddIsTruncatingMutation} from "../../../generated/graphql";
...
const TruncatingEditor : React.FC<Props> = ({isTruncating, id,field,variant_ID,ref_array,set_editing,refetch}) => {
    ...
    const [addIsTruncatingMutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useVariantRegionAddIsTruncatingMutation({variables:{variant_id:'',  old_eb_id:'',
    ...
    async function extracted(refs: string[]) {
        ...
        await addIsTruncatingMutation({variables: mutation_object})
    }
    ...
    const state  = {
        selectedOption: { value: isTruncating, label: get_label_for_value(isTruncating) },
    };
...
export default TruncatingEditor;