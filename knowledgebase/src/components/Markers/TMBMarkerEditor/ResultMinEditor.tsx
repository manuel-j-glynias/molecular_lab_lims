...
import {Scalars, useTmbMarkerAddResultMinMutation} from "../../../generated/graphql";
...
const ResultMinEditor : React.FC<Props> = ({minResult,set_editing,es_ID, es_field,marker_id,ref_array,refetch}) => {

    const [statement_value, set_statement_value] = React.useState(minResult);
    const statement_string = React.useRef(minResult.toString());
    const [pmid_list, set_pmid_list] = React.useState('');

    const [addResultMinMutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useTmbMarkerAddResultMinMutation(
        {variables:
                    {marker_id:'',
                    old_es_id:'',
                    date: '',
    ...
    return ( <div className="form-group">
            <textarea className={`${className}__ShortTextarea`} name="statement" placeholder="Min Value(float)" value={get_statement_value()} onChange={(e) => {handle_change(e.target.value)}}/>
