...
import {useVariantSnvIndelAddGDotMutation} from "../../../generated/graphql";
...
const GDotEditableStatementEditor : React.FC<Props> = ({statement,set_editing,es_ID, es_field,variant_ID,ref_array,refetch}) => {
    ...
    const [addGDotMutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useVariantSnvIndelAddGDotMutation({variables:{variant_id:'', old_es_id:'',
    ...
    async function call_mutation(pmids: Array<string>) {
        ...
        await addGDotMutation({variables: mutation_object})
    }
    ...
    return ( <div className="form-group">
        <textarea className={`${className}__ShortTextarea`} name="statement" placeholder="gDot" value={get_statement_value()} onChange={(e) => {handle_change(e.target.value)}}/>
...
export default GDotEditableStatementEditor;