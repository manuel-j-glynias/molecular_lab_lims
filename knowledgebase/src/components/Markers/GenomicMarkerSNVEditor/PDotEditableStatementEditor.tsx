...
import {useVariantSnvIndelAddPDotMutation} from "../../../generated/graphql";
...
const PDotEditableStatementEditor : React.FC<Props> = ({statement,set_editing,es_ID, es_field,variant_ID,ref_array,refetch}) => {
    ...
    const [addPDotMutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useVariantSnvIndelAddPDotMutation({variables:{variant_id:'', old_es_id:'',
    ...
    async function call_mutation(pmids: Array<string>) {
        const mutation_object = get_description_mutation_object(variant_ID, es_ID, es_field, statement_string.current, user_ID, pmids)
        await addPDotMutation({variables: mutation_object})
    }
    ...
    return ( <div className="form-group">
        <textarea className={`${className}__ShortTextarea`} name="statement" placeholder="pDot" value={get_statement_value()} onChange={(e) => {handle_change(e.target.value)}}/>
...
export default PDotEditableStatementEditor;