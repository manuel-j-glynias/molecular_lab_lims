...
import {useVariantSnvIndelAddExonMutation} from "../../../generated/graphql";
...
const ExonEditableStatementEditor : React.FC<Props> = ({statement,set_editing,es_ID, es_field,variant_ID,ref_array,refetch}) => {
    ...
    const [addExonMutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useVariantSnvIndelAddExonMutation({variables:{variant_id:'', old_es_id:'',
    ...
    async function call_mutation(pmids: Array<string>) {
        ...
        await addExonMutation({variables: mutation_object})
    }
    ...
    return ( <div className="form-group">
        <textarea className={`${className}__ShortTextarea`} name="statement" placeholder="Exon" value={get_statement_value()} onChange={(e) => {handle_change(e.target.value)}}/>
...
export default ExonEditableStatementEditor;