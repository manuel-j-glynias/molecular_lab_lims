...
import {useVariantFusionAddGene3PrimeMutation} from "../../../generated/graphql";
...
const Gene3Editor : React.FC<Props> = ({gene3_id,set_editing,es_ID, es_field,variant_ID,ref_array,refetch}) => {

    const [pmid_list, set_pmid_list] = React.useState('');
    const [gene_id_value, set_gene_id_value] = React.useState(gene3_id);
    ...
    const [addGene3Mutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useVariantFusionAddGene3PrimeMutation({variables:{variant_id:'', old_id:'',
    ...
    async function call_mutation(pmids: Array<string>) {
        ...
        await addGene3Mutation({variables: mutation_object})
    }
