...
import {Scalars, useVariantFusionAddExon3PrimeMutation} from "../../../generated/graphql";
import {get_exon3_mutation_object} from "../../common/Helpers/EditableStatementHelper";
...
const Exon3Editor : React.FC<Props> = ({exon3,set_editing,es_ID, es_field,variant_ID,ref_array,refetch}) => {

    const [statement_value, set_statement_value] = React.useState(exon3);
    const statement_string = React.useRef(exon3==-1 ? '' : exon3.toString());
    const [pmid_list, set_pmid_list] = React.useState('');

    const [addExon3Mutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useVariantFusionAddExon3PrimeMutation({variables:{variant_id:'', old_id:'',
            date: '', field: '', exon3:-1, id: '', user_id: '', ref_aray:[]}})
    ...
    async function call_mutation(pmids: Array<string>) {
        const mutation_object = get_exon3_mutation_object(variant_ID, es_ID, es_field, getExon3(), user_ID, pmids)
        await addExon3Mutation({variables: mutation_object})
    }
