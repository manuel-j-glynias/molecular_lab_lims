...
import {useProteinExpressionMarkerAddMethodMutation} from "../../../generated/graphql";
...
const MethodEditableStatementEditor : React.FC<Props> = ({statement,set_editing,es_ID, es_field,marker_id,ref_array,refetch}) => {
    ...
    const [addMethodMutation, { loading: mutationLoading, error: mutationError, data:mutationData }] = useProteinExpressionMarkerAddMethodMutation({variables:{marker_id:'', old_es_id:'',
    ...
    async function call_mutation(pmids: Array<string>) {
        ...
        await addMethodMutation({variables: mutation_object})
    }
    ...
    return ( <div className="form-group">
        <textarea className={`${className}__ShortTextarea`} name="statement" placeholder="Method" value={get_statement_value()} onChange={(e) => {handle_change(e.target.value)}}/>
            <div className="form-group">
                <div><input className={`${className}__PMID_Text`} type="text" placeholder="PMIDs" value={pmid_list} onChange={e => set_pmid_list(e.target.value )} required/></div>
...
export default MethodEditableStatementEditor;