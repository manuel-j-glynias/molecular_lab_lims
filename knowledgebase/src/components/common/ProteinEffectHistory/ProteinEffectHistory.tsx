...
import { EditableProteinEffectQuery } from '../../../generated/graphql'
import './styles.css'


export interface Props {
    data: EditableProteinEffectQuery;
}
...
const ProteinEffectHistory: React.FC<Props> = ({data}) => (

    <div className={className}>
        <h3>History</h3>
        <ol className={`${className}__list`}>
            {!!data.EditableProteinEffect &&
            data.EditableProteinEffect.map(
                (ES, i: string | number ) =>
                    !!ES && (
...
export default ProteinEffectHistory;