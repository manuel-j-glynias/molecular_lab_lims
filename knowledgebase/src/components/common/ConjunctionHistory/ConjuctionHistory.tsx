...
import { EditableOmniConjunctionQuery } from '../../../generated/graphql'
import './styles.css'


export interface Props {
    data: EditableOmniConjunctionQuery;
}
...
const ConjuctionHistory: React.FC<Props> = ({data}) => (

    <div className={className}>
        <h3>History</h3>
        <ol className={`${className}__list`}>
            {!!data.EditableOmniConjunction &&
            data.EditableOmniConjunction.map(
                (ES, i: string | number ) =>
                    !!ES && (
...
export default ConjuctionHistory;