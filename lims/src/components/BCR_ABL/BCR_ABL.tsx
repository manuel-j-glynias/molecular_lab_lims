...
import {Bcr_AblCasesListQuery} from '../../generated/graphql';
import './styles.css'


interface Props {
    data: Bcr_AblCasesListQuery;
    filter_term: string;
...
const BCR_ABL: React.FC<Props> = ({data, filter_term, set_filter_term,
                ...
                <div className={`${className}__Table_Container`}>
                    ...
                    <div className={`${className}__Table`}>
                        {!!data.Case && data.Case.map((c, i: number) =>
                            !!c && c.order.test_ordered.name==='Xpert BCR-ABL' && (
